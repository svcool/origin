...
private:
	...
	std::string* arr;
public:
	//void set_address(std::ifstream);//прототип чтени€ данных из файла
	//void get_output_address_back(std::ofstream); // прототип вывод а адреса в файл в обратном пор€дке в строки
	...
	void set_address(std::ifstream& f_in, std::string*& arr, int& size) {
		...
		else std::cout << "ќшибка открыти€ файла" << std::endl;
	}
	...
	void get_output_address_back(std::ofstream& f_out, std::string* arr, int size) {
		if (f_out.is_open()) {
			int k{}; // дл€ считывани€ 4 данных адреса
			f_out << size << "\n";
			for (int i{}; i < size; i++) {
				//
