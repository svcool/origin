...
{
public:
	virtual void setNextHandler(AuthenticationHandler* handler) = 0; //  эта функция позволяет установить следующий обработчик в цепочке.
	virtual void handleRequest(const std::string& request)	= 0; // эта функция отвечает за обработку запросов аутентификации.
};
