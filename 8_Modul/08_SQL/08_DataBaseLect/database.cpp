...
void DataBase::AddDataBase(QString driver, QString nameDB)
{
    ...
    *dataBase = QSqlDatabase::addDatabase(driver, nameDB);
     // *dataBase = QSqlDatabase::addDatabase(driver, nameDB1); //можно добавить несколько БД
}
    ...
    */
    ...
    case requestHorrors:
    {
        ...
        */
        ...
        tableWidget->setColumnCount(3); //количество столбцов в запросе
        tableWidget->setRowCount(0);
        QStringList hdrs;
        hdrs << "Название" << "Год выпуска" << "Жанр";
        tableWidget->setHorizontalHeaderLabels(hdrs);//устанавливает виджет

        uint32_t conterRows = 0;//счетчик строк, таблица читается построчно

        while(simpleQuery->next()){//цикл по строкам ответа
            QString str;
            tableWidget->insertRow(conterRows);

            for(int i = 0; i<tableWidget->columnCount(); ++i){ //цикл по столбцам ответа

                str = simpleQuery->value(i).toString();
                QTableWidgetItem *item = new QTableWidgetItem(str); //классы таблиц
                tableWidget->setItem(tableWidget->rowCount() - 1, i, item); //координаты
                //таблица -> координаты строка столбец, размещем item
            }
            ++conterRows;
        }
...
{
    return dataBase->lastError(); //isValid() проверка валидности ошибки
}
