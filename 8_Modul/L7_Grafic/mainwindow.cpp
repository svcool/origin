    ...
    , ui(new Ui::MainWindow)
{
    ...
    chart = new QChart(); //иницилизация графика

    chartView = new QChartView(chart); //для отображения графика

    chartWindow = new QMainWindow(this);// экземпляр окна для графика
...
QVector<double> MainWindow::ProcessFile(const QVector<uint32_t> dataFile)
{
    ...
    foreach (int word, dataFile) {
        ...
        //наполнение вектора первую секунду
        if(time.elapsed() <= 1){
        resultDataTime.append(res);
        }
...
void MainWindow::on_pb_start_clicked()
{
    ...
    auto findMax = [&](QVector<double> res){
                                                ...
                                                ptrGraph->clear();
                                                for(int i = 0; i < FD; ++i) {
                                                     ptrGraph->append(i, res[i]);
                                                }

                                                // Вызов сигнала для обновления графика
                                                emit sig_finish();
...
void MainWindow::UpdateGraph(){
    ...
    if(chart->series().isEmpty() == false){
        ...
        // Удаляем эту серию из графика или будет ругаться
        chart->removeSeries(ptrGraph);
    }
    chartWindow->setCentralWidget(new QChartView(chart)); //помещаем chart в центр окна
    chartWindow->resize(400, 300); //размер окна
    chart->legend()->hide(); //скрыть легенду
    chart->addSeries(ptrGraph); // добавление серии на график
